                ...
                {{-- <div class="col-md-6">
                   ...
                   <div class="col-md-6">
                    ...
                    <select id="entity_id" name="entity_id" class="form-select select2" required>
                        ...
                        <option value="{{ $parentEntity['id'] }}" {{ (old('entity_id', $user['entity_id']) == $parentEntity['id']) ? 'selected' : '' }}>
                            {{ $parentEntity['name'] }}
                        </option>
                    @endforeach
                    </select>
               ...
               @endif
                ...
                <div class="col-md-6" id="testTypeDiv" style="display: none;">
                    ...
                    <select id="test_type" name="test_type" class="form-select select2">
                        ...
                        <option value="1" {{ (old('test_type', $user['test_type']) == '1') ? 'selected' : '' }}>NAT</option>
                        <option value="2" {{ (old('test_type', $user['test_type']) == '2') ? 'selected' : '' }}>ELISA</option>
                        <option value="3" {{ (old('test_type', $user['test_type']) == '3') ? 'selected' : '' }}>BOTH</option>
                    </select>
                </div>
        ...
        $(document).ready(function() {      
           ...
           // var urlcityByStateIdTemplate = "{{ route('api.cities', ['stateId' => '__STATE_ID__']) }}";
            ...
            function updateTestTypeVisibility() {
                ...
                console.log('Updating test type visibility for role:', roleId);
                if (roleId == 16) {
                    testTypeDiv.show();
            ...
            roleDropdown.on('change', updateTestTypeVisibility);

            // Initialize test type visibility based on current role selection
            updateTestTypeVisibility();

            // Log the current role and test type values for debugging
            console.log('Current role:', roleDropdown.val());
            console.log('Current test type:', testTypeSelect.val());
            console.log('Test type div visible:', testTypeDiv.is(':visible'));
